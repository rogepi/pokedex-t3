...
export default function PokeCard({ pokemon }: IPokeCardProps) {
  return (
    ...
    >
      <div className="w-24 h-24 md:w-30 md:h-30 lg:w-48 lg:h-48 relative">
      <Image src={pokemon.imgUrl} alt={pokemon.imgUrl} fill 
        sizes="(max-width: 768px) 100vw,
        (max-width: 1024px) 50vw,
        33vw"/>
        </div>
      <span className="font-bold text-gray-300">{'#'+pokemon.index}</span>
      <span className="text-md md:text-xl lg:text-2xl font-bold">{pokemon.nameZh}</span>
      <div className="flex gap-2">
        <AttrIcon type={pokemon.type1} />
        {pokemon.type2?
        <AttrIcon type={pokemon.type2} />:''
      }
        </div>
    </Link>
  )
